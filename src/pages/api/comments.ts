...
export const GET: APIRoute = async ({ url, locals, cookies }) => {
  ...
  if (url.searchParams.get('all') === '1') {
    ...
    return new Response(JSON.stringify(await getAllComments(kv)), { headers: { 'Content-Type': 'application/json' } });
  }
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    await addComment(kv, slug, comment);
...
export const DELETE: APIRoute = async ({ request, locals, cookies }) => {
  ...
  try {
    ...
    await deleteComment(kv, slug, id);
